...
add_language (const char *name, int loc)
{
  ...
  get_shvar_value (langfile, buf, langs[loc].name);

  sprintf (buf, "%s_font", name);
  get_shvar_value (langfile, buf, langs[loc].font);

  sprintf (buf, "%s_bitmapmenu", name);
  if (!get_shvar_value (langfile, buf, buf))
...
init_langconf (void)
{
  ...
  if (get_shvar_value (langfile, "LANGUAGES", buf) || buf[0] == '\0')
    {
  ...
  get_shvar_value (langfile, "DEFAULT_LANGUAGE", buf);
  for (i = 0; i < langcount; i++)
    if (!strcmp (langs[i].shortname, buf))
      deflang = &langs[i];

  if (get_shvar_value (langfile, "FONTS", buf) || buf[0] == '\0')
    {
